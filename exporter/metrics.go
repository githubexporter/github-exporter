...
func AddMetrics() map[string]*prometheus.Desc {
	...
	APIMetrics["Visibility"] = prometheus.NewDesc(
		prometheus.BuildFQName("github", "repo", "visibility"),
		"Visibility for given repository, [private - 0, public - 1]",
		[]string{"repo", "user", "fork", "archived", "license", "language"}, nil,
	)
...
func (e *Exporter) processMetrics(data []*Datum, rates *RateLimits, ch chan<- prometheus.Metric) error {
	...
	for _, x := range data {
		...
		if x.Private {
			ch <- prometheus.MustNewConstMetric(e.APIMetrics["Visibility"], prometheus.GaugeValue, 0, x.Name, x.Owner.Login, strconv.FormatBool(x.Private), strconv.FormatBool(x.Fork), strconv.FormatBool(x.Archived), x.License.Key, x.Language)
		} else {
			ch <- prometheus.MustNewConstMetric(e.APIMetrics["Visibility"], prometheus.GaugeValue, 1, x.Name, x.Owner.Login, strconv.FormatBool(x.Fork), strconv.FormatBool(x.Archived), x.License.Key, x.Language)
		}
